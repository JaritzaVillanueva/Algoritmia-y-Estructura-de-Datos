...
struct NodoArbol * crearNuevoNodoArbol(struct NodoArbol * arbolIzquierdo, 
                               int lote, int cant, struct NodoArbol * arbolDerecho){
...
void plantarArbolBinario(struct ArbolBinario & arbol, struct NodoArbol * arbolIzquierdo,
    ...
    struct NodoArbol * nuevoNodo = crearNuevoNodoArbol(arbolIzquierdo, lote, cant, arbolDerecho);
    arbol.raiz = nuevoNodo;
}

void plantarArbolBinario(struct ArbolBinario & arbol, struct ArbolBinario arbolIzquierdo,
                         int lote, int cant, struct ArbolBinario arbolDerecho){
    
    struct NodoArbol * nuevoNodo = crearNuevoNodoArbol(arbolIzquierdo.raiz, lote, cant, arbolDerecho.raiz);
    arbol.raiz = nuevoNodo;
}
...
void imprimeNodo(struct NodoArbol * nodo){
    cout<<setw(10)<<nodo->lote<<" - "<<nodo->cant;
}
...
int sumarNodosRecursivo(struct NodoArbol * nodo){
    if(esNodoVacio(nodo))
        ...
        return nodo->cant + sumarNodosRecursivo(nodo->izquierda) + sumarNodosRecursivo(nodo->derecha);
}
