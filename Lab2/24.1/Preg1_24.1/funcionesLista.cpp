...
void insertarencola(struct Lista & tad, int elemento, const char *cod) {
    ...
    struct Nodo * nuevoNodo = crearNodo(elemento, cod, nullptr);
...
struct Nodo * crearNodo(int elemento, const char *cod, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
    strcpy(nuevoNodo->codigo, cod);
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            int dd,mm,aa;
            separarFecha(recorrido->elemento, dd, mm, aa);
            cout << setfill('0')<<right<<setw(2)<<dd<<"/"<<setw(2)<<mm<<"/"<<setw(4)
                    <<aa<<setfill(' ')<<" "<<recorrido->codigo;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
