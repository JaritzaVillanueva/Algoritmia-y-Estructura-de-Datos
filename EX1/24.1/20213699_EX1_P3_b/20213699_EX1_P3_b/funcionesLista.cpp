...
void insertarencola(struct Lista & tad, struct Avion avion) {
    ...
    struct Nodo * nuevoNodo = crearNodo(avion, nullptr);
...
//        cout << "[";
        ...
        while (recorrido != nullptr) {
            ...
            int hhIni, hhNueva, mmIni, mmNueva;
            separarHora(recorrido->avion.minIniTotal, hhIni, mmIni);
            separarHora(recorrido->avion.minIniNueva, hhNueva, mmNueva);
            cout << left<<setw(10)<<recorrido->avion.numero<<right<<setw(6)<<hhIni<<":"
                    <<setfill('0')<<setw(2)<<mmIni<<setfill(' ')<<setw(15)<<hhNueva<<":"
                    <<setfill('0')<<setw(2)<<mmNueva<<setfill(' ');
            recorrido = recorrido->siguiente;
        }
        cout<< endl;
    }
